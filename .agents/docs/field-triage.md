...
### Recover publication with newer release tooling

When a validated candidate contains an obsolete release workflow or gate, use the manual `workflow_dispatch` entry of `Release (tag)` in `.github/workflows/release.yml` from `main`. Its `candidate_sha`, `candidate_run_id`, and `tag` identify the original candidate, not the newer tooling commit. Run first with `publish: false` to verify the original CI artifact, version history, release notes, and field authority without creating a tag or publishing. After reviewing that run, an owner-authorized invocation with `publish: true` may publish it. Do not move the tag to the tooling commit or copy field status onto that commit.

Recovery requires a successful main push of `.github/workflows/ci.yml`, its unique unexpired exact-SHA candidate artifact, matching manifest and tarball digest, and an ancestor candidate of the executing tooling SHA. It runs the newer Changesets and field gates against a clean detached checkout of the original candidate. An existing tag must resolve to that candidate; a missing tag is created only after verification. The workflow's GitHub token creates the tag without triggering a second tag-push publication. The tag and manual jobs share publication concurrency in the same workflow.

Publication uses the original downloaded tarball without rebuilding, packing, or running package lifecycle scripts. The signed provenance identifies the candidate source, original CI artifact and run, and the actual recovery tooling/workflow run separately. The package digest must match that attestation. Recovery revalidates the artifact, tag, and exact-candidate field authority immediately before publishing; an expired artifact, mismatched tag, or invalidated field evidence stops it. GitHub release notes come from the candidate's changelog, and an existing release is preserved. A successful verification-only run does not claim OIDC signing, registry authentication, or publication has succeeded; those require the authorized publishing run.

The newer tooling SHA requires its own PR checks but is not the product being released and does not inherit the candidate's field status.
