...
import type {
  AxisDef, Blueprint, EmitDef, Framework, OwnedPrimitive, PlaybookSection,
} from '../config';
...
function principles(): Blueprint['principles'] {
  return [
    { id: 'by-responsibility', say: 'Split by responsibility, not by size', why: 'The signal to '
      + 'split is how many things a unit does — line count is only a backstop.', land: 'claude' },
    { id: 'single-source-of-truth', say: 'One source of truth', why: 'Derive computed values; '
      + 'never store duplicate mutable state that can desync.', land: 'claude' },
    { id: 'narrow-interfaces', say: 'Keep interfaces narrow', why: 'Narrow inputs and outputs so '
      + 'illegal states cannot be expressed.', land: 'claude' },
    { id: 'knowledge-where-used', say: 'Keep knowledge where it is used', why: 'Push derivation to '
      + 'the child and state to its lowest common owner; do not hoist.', land: 'claude' },
    { id: 'dead-code', say: 'Dead code: delete it or mark it', why: 'An abstraction with no '
      + 'consumer is dead; sweep orphans, mark retained-dead as deprecated.', land: 'claude' },
    { id: 'lint-is-triage', say: 'Lint is an entry point, not a verdict', why: 'Mechanical checks '
      + 'only triage; cohesion and invariants need review.', land: 'claude' },
    { id: 'ac-not-scripture', say: 'Acceptance criteria are a start, not scripture', why: 'Fixing '
      + 'a ticket that violates an abstraction\'s '
      + 'responsibility is upholding the design.', land: 'claude' },
    { id: 'yagni', say: 'YAGNI — do not over-engineer', why: '"Might need it later" is '
      + 'not a reason to abstract now.', land: 'claude' },
    { id: 'cost-is-a-dimension', say: 'Cost is the third dimension', why: 'Cost = work per event × '
      + 'event frequency; price any logic wired to a data source.', land: 'claude' },
  ];
}
...
function componentShape(): AxisDef[] {
  return [
    ...
    },
    {
      ...
      why: 'Three moves: split a multi-concern unit; collapse parallel raw states carrying an '
        + 'invariant into one modeled state; merge symmetric twins into one object of the same '
        + 'shape. Count and size are weak signals — '
        + 'whether the state is modeled is the review call.',
      triage: 'max-params',
    },
    {
    ...
    },
    {
      ...
      why: 'Route/id resolution, the loading shell, shared sources, cross-child lifecycle — '
        + 'never deriving values on behalf of each child. '
        + 'Field-tested: a 6666-line detail page down to 552.',
      triage: 'max-lines',
    },
    {
    ...
    },
    {
      ...
      why: 'The caller receives a unit that is already running and cleans itself up — '
        + 'not a kit of handlers to wire into mount/effect hooks. '
        + 'Field-tested: 19 exports down to a one-line call.',
    },
    {
      id: 'pure-helpers',
      name: 'Pure Helpers ≠ Composables',
      say: 'Keep pure functions out of reactive/lifecycle units.',
      why: 'One exported function does not demand one file: responsibility splits at the function '
        + 'level; the file splits only when max-lines approaches. '
        + 'Expose the decision a unit makes, not its raw ingredients.',
    },
  ];
}

/** Playbook section 1 of 3. */
    ...
    rules: [
      ...
      },
      {
        ...
        why: 'Patch the changed entry and keep container identity; '
          + 'whole-replace is for baseline rebuilds only. A prop whose identity changed while '
          + 'its value did not is the disease. Write shapes do not port across frameworks.',
      },
      {
        id: 'render-diagnosis',
        say: 'Diagnose re-renders in four steps, never by guessing.',
        why: 'Who renders (profiler) → what triggered it (render tracing) → who produced the '
          + 'identity (grep the assignment sites) → was it '
          + 'worth it (compare against the event payload).',
      },
      {
        id: 'measurable-perf',
  ...
  return {
    ...
    rules: [
      ...
      },
      {
        ...
        why: 'A safety net asserting values the arc is about to '
          + 'change fails the moment the sibling refactor lands.',
      },
      {
        id: 'contract-test-payloads',
        say: 'AC-named payload fields deserve a contract test.',
        why: 'Asserting that the mocked service receives field X is not a tautology — a dropped '
          + 'field or an unbound handler breaks it while the source constant stays green.',
      },
      {
        id: 'summarize-with-themes',
        say: 'Wrap an arc with cross-cutting themes and verified numbers.',
        why: 'Name the forces (ownership inversion, IO shrinkage, '
          + 'SRP) and attach before/after numbers verified against git history.',
      },
    ],
  };
}

/** Playbook section 3 of 3. */
    ...
    rules: [
      {
        ...
        why: 'State the principle being protected, show how the literal ticket reading '
          + 'violates it, and present the choice as that principle\'s natural consequence.',
      },
      {
  ...
  return defineBlueprint({
    ...
    architecture: {
      ...
      layers: [
        ...
        {
          name: 'containers',
          does: 'A feature: assembles components, owns local state, '
            + 'calls services, drives navigation.',
        },
        {
          name: 'components',
        ...
        {
          name: 'hooks',
          does: 'Adapts server and shared state; '
            + 'the only layer that injects context or owns a store.',
          owns: owns.hooks,
        },
        {
          name: 'contexts',
