...
describe('canonical module-first runway projection', () => {
  it.each(factories)(
    'keeps %s governance while projecting only topology',
    (_framework, factory) => {
  ...
  it.each(factories)(
    'maps container permissions to the module root for %s',
    (_framework, factory) => {
      ...
      expect(
        blueprint.architecture.layers.flatMap(({ allowedImporters = [] }) => allowedImporters),
