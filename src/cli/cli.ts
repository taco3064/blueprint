...
const IMPACT_HELP = [
  ...
  '',
  'Informational, never a gate: the exit code is 0 whatever the count. Two',
  'special rows: `parse-error` means a file could not be parsed (missing parser',
  'dep?) and its numbers cannot be trusted until fixed; `unused-disable-directive`',
  'means an inline eslint-disable suppresses nothing HERE — either genuinely',
  'stale (common after rule renames), or referencing one of your own rules that',
  'this isolated run does not carry. Confirm against your full lint before',
  'deleting. Rules that are not blueprint\'s at all (hit via existing comments)',
  'render in a separate section and never count toward the total.',
  '',
