...
import importsPlugin from 'eslint-plugin-import-lite';
...
function loader(eslintModule: unknown) {
  ...
  const loadModule = async (name: string): Promise<unknown> => {
    ...
    if (name === 'eslint-plugin-import-lite') return { default: importsPlugin };
...
describe('runImpact', () => {
  ...
  it('keeps a plain-JS Vue stack on the vue parser alone', async () => {
    ...
    expect(loaded).toContain('eslint-plugin-import-lite');
  ...
  it('measures the injected-plugin gates instead of silently reporting zero', async () => {
    ...
    expect(gate?.rules?.['import-lite/no-duplicates']).toBeDefined();
    expect(gate?.plugins?.['@stylistic']).toBe(stylisticPlugin);
    expect(gate?.plugins?.['import-lite']).toBe(importsPlugin);
  });
