...
export const STRUCTURAL_RULES: StructuralRule[] = [...STRUCTURAL_RULE_DESCRIPTIONS];
...
function gateSpecs(): GateSpec[] {
  return [
    ...METRIC_GATES.map((gate) => ({
      ...
      note: renderMetricGateNote(gate.wrap),
...
function layerBans(blueprint: Blueprint): LayerBans[] {
  ...
  const positioned = resolved.layerPositions.map((position) => {
    ...
    return {
      ...
      packages,
      ...(packages.length ? { packagesNote: renderPackagesNotCompared().join(' ') } : {}),
      globals: globalRules
        .filter((rule) => !rule.allowedIn.includes(layer.name))
      ...
      selfOnly: resolved.selfOnlyTargets(layer.name).map((target) => {
        ...
        return {
          ...
          note: renderSelfOnlyMessageNote(),
        };
      }),
  ...
  const containers = resolved.modules.map((module): LayerBans => ({
    ...
    ...(containerPackages.length
      ? { packagesNote: renderPackagesNotCompared().join(' ') }
      : {}),
