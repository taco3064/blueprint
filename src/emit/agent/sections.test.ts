...
describe('renderPlacement', () => {
  ...
  it('drops the private clause when a folder module has none', () => {
    const out = renderPlacement(arch({ module: { layout: 'folder', entry: 'index',
      private: [] } }));
...
describe('renderHardRules', () => {
  it('includes entry-only for folder layout and lists error-tier gates', () => {
    ...
    expect(out).toContain('- Import only from downstream layers — '
      + 'never upstream, never the same layer.');
  ...
  it('omits entry-only for flat layout', () => {
    const out = renderHardRules(arch({ module: { layout: 'flat', entry: 'index', private: [] } }),
      undefined);
...
describe('renderComponentShape (contract)', () => {
  const axes: AxisDef[] = [
    { id: 'a', name: 'IO Shrinkage', say: 'Narrow IO.', why: 'Model the state.',
      triage: 'max-params' },
    { id: 'b', name: 'Orchestration Shell', say: 'Pages orchestrate.',
  ...
  it('renders one directive bullet per axis, with triage as entry point only', () => {
    ...
    expect(out).toContain('- **IO Shrinkage** — Narrow IO. Model the state. '
      + '(triage: `max-params` is an entry point, never the verdict)');

    expect(out).toContain('- **Orchestration Shell** — Pages orchestrate. '
      + 'No per-child derivation.');
  });
});
...
describe('renderCompactContract', () => {
  ...
  it('lists only ids a machine actually gates', () => {
    ...
    const out = renderCompactContract(blueprint({
      rules: { maxLines: { tier: 'error' as const, value: 300 }, noUtils: 'error',
        deadCode: 'error' },
    }));
