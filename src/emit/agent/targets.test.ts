...
describe('emitAgentFiles', () => {
  ...
  it('maps every target to its tool file', () => {
    const files = emitAgentFiles(
      bp(['claude', 'agents', 'gemini', 'copilot', 'cursor', 'windsurf']),
    );
  ...
  it('escapes quotes in the name and falls back to "Project" without one', () => {
    ...
    expect(emitAgentFiles(quoted)[0].content).toContain(
      'description: "My \\"X\\" architecture contract"',
    );

    const unnamed: Blueprint = { ...vuePreset(), name: undefined, emit: { agents: ['cursor'] } };

    expect(emitAgentFiles(unnamed)[0].content).toContain(
