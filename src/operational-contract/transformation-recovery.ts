...
export function renderTransformationRecoveryGuide(origin: {
  ...
  return operationalText([
    ...
    'in target.decisions using the complete shape below. The top-level `source` equals the',
    'corresponding `origin.sources[].unit`; `unit` is not a decision property:', '',
    '```json',
    '{',
    '  "source": "pages/Game",',
    '  "destinations": ["src/app/Game.tsx"],',
    '  "members": [',
    '    { "source": "src/pages/Game/Game.tsx", "destination": "src/app/Game.tsx" }',
    '  ]',
    '}',
    '```', '',
    'Destinations are exact member destination file paths, never directories. Preserve',
