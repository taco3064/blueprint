...
export function renderSemantics(claudeLauncher: boolean): string {
  return [
    ...
    `- **Test-file policy:** ${renderTestFilesOperational('core', 'en')}`,
...
export function renderRuleCatalog(facts: AuthoringRuleCatalogFact): string {
  const { metricGates, pluginGates, documentationOnlyRules } = facts;
  ...
  return [
    ...
    '',
    `${metricGates.map((gate) => `- \`${gate.id}\` → \`${gate.rule}\` (default ${gate.fallback})`).join('\n')}`,
    `${pluginGates.map((gate) => `- \`${gate.id}\` → \`${gate.emits}\` — ${gate.note}`).join('\n')}`,
    '',
    '**Documentation-only ids — never an ESLint line:**',
    '',
    `${documentationOnlyRules.map((entry) => `- \`${entry.id}\` — ${entry.note}`).join('\n')}`,
  ].join('\n');
}

export function renderSchemaSketch(sourceRoot: string): string {
  const sharedRoot = sourceRoot === '.' ? 'shared' : `${sourceRoot}/shared`;
  ...
  return [
    ...
    `    additionalAliases: { '~shared': './${sharedRoot}' },`,
    '    layers: [',
    ...
    '    ],',
    '    // No `modules` key: its presence, even `modules: []`, selects module-first. This',
    '    // playbook authors the layer-first topology that was selected.',
    '    // layerFiles is optional and defaults to framework-aware globs.',
...
export function renderAcceptanceGates(
  ...
  return [
    ...
    '- [ ] `npx blueprint inspect --baseline` exits 0 — it validates the ledger written by the '
    + 'earlier `--update-baseline` step; it never writes one. The ledger is locked when debt '
    + 'exists and correctly absent when it does not',
...
export function renderSurveyEvidence(renderedSurvey: string): string {
  return [
    '',
    '## Survey evidence',
    '',
