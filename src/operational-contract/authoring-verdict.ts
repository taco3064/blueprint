...
function renderBuildChoice(viteTs: ViteTsFact | null, buildCommand: string): string[] {
  const shared = [
  ...
  return [
    ...
    `   Run them as two commands rather than one \`${buildCommand}\`: together they cover the same ground, and only the split lets you report which edit each one verified — fused into one result, "the tsconfig edit type-checks" and "the vite config loads" arrive as a single green you then have to hedge.`,
    ...shared,
  ];
}

function renderBuildArtifacts(tscOut: TscArtifactFact | null): string {
  if (tscOut !== null) {
    return [
...
function renderEarlyExitVerify(
  viteTs: ViteTsFact | null,
  tscOut: TscArtifactFact | null,
  ...
  return [
    ...
    `   Either way, close this step by running the project's own lint once (\`${commands.lint}\`, or \`npx eslint .\` without a script): doctor now runs a safely replayable terminal ESLint leg, but never the other script segments, so the full command is still their proof.`,
    ...
    ...renderBuildChoice(viteTs, commands.build),
...
export function renderVerdict(
  ...
  if (survey.scopeRequired) {
    return [
      ...
      'Run `npx blueprint survey --source-root <application-source-root>`, then set the same '
      + 'path as '
  ...
  return [
    ...
    renderEarlyExitVerify(viteTs, tscOut, commands),
