...
import type { OperationalLocale, TestFilesSurface } from './test-files';
...
it('keeps every operational surface dispatch distinct', () => {
  const editions: TestFilesSurface[] = [
    'agent-placement',
    'core',
    'deps',
    'merge-scope',
  ...
  for (const locale of ['en', 'zh-TW'] as const) {
    const rendered = editions.map((edition) => renderTestFilesOperational(edition, locale));

    expect(new Set(rendered)).toHaveLength(editions.length);
  }
});

describe('architecture.testFiles operational contract', () => {
  it('renders every edition and locale for both matching and empty policies', () => {
    ...
    const locales: OperationalLocale[] = ['en', 'zh-TW'];

    expect(TEST_FILES_OPERATIONAL_NODE).toBe('architecture.testFiles');

    for (const edition of editions) {
      for (const locale of locales) {
        expect(renderTestFilesOperational(edition, locale)).toBeTruthy();
        expect(renderTestFilesOperational(edition, locale, [])).toBeTruthy();
      }

      expect(renderTestFilesOperational(edition, 'en'))
        .not.toBe(renderTestFilesOperational(edition, 'zh-TW'));

      expect(renderTestFilesOperational(edition, 'en')).not.toMatch(/\p{Script=Han}/u);
      expect(renderTestFilesOperational(edition, 'zh-TW')).toMatch(/\p{Script=Han}/u);
  ...
  it('renders measured reach and all of its localized fact fields', () => {
    ...
    for (const locale of locales) {
      expect(renderEmptyTestFilesOperational(locale)).toBeTruthy();

      const complete = renderUnreachedTestFilesOperational(locale, {
      ...
      expect(renderUnreachedTestFilesOperational(locale, {
        deadGlobs: ['**/*.check.ts'],
        allGlobsDead: false,
        outsideScan: [],
        undecidedGlobs: [],
  ...
  it('feeds lint, inspect, and both localized editions from one resolved policy', () => {
    ...
    for (const locale of ['en', 'zh-TW'] as const) {
      const edition = renderResolvedTestFilesOperational('core', locale, policy);
  ...
  it('makes an empty owner move behavior and both editions together', () => {
    ...
    expect(renderResolvedTestFilesOperational('core', 'en', empty)).toContain('exempts nothing');

    expect(renderResolvedTestFilesOperational('core', 'zh-TW', empty))
      .toContain('不會讓任何檔案豁免');
  });
});

it('keeps agent placement locale direction observable', () => {
  expect(renderTestFilesOperational('agent-placement', 'en'))
    .toContain('Test support matching');

  expect(renderTestFilesOperational('agent-placement', 'zh-TW'))
    .toContain('測試支援檔');
