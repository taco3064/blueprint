import { printConfigCaveats } from './authoring-playbook';
...
function renderTestExemptions(): string {
  return [
    ...
    + 'lint stays green while the files it was never meant to reach are quietly under it.',
    '     Doctor compares selectors, not scope, so nothing downstream catches this.',
    '     The same move runs the other way when YOUR rule had no matching exemption: '
    ...
    + 'since it still governs its original scope and nothing has to be given up.',
