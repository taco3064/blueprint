...
export const TEST_FILES_OPERATIONAL_NODE = 'architecture.testFiles';

export type OperationalLocale = 'en' | 'zh-TW';

export type TestFilesSurface
  = | 'agent-placement'
    | 'core'
    | 'deps'
    | 'merge-scope'
...
type TestFilesRenderer = (
  policy: ResolvedTestFiles,
  locale: OperationalLocale,
) => string;

const EDITION_RENDERERS: Record<TestFilesSurface, TestFilesRenderer> = {
...
export function renderTestFilesOperational(
  edition: TestFilesSurface,
  locale: OperationalLocale,
  testFiles?: ArchitectureDef['testFiles'],
): string {
  return renderResolvedTestFilesOperational(edition, locale, resolveTestFiles(testFiles));
}

export function renderResolvedTestFilesOperational(
  edition: TestFilesSurface,
  locale: OperationalLocale,
  policy: ResolvedTestFiles,
): string {
...
function renderCore(policy: ResolvedTestFiles, locale: OperationalLocale): string {
  if (locale === 'zh-TW') {
...
function renderDeps(policy: ResolvedTestFiles, locale: OperationalLocale): string {
  if (locale === 'zh-TW') {
    return policy.architectureExemptions.length
...
function renderSurvey(policy: ResolvedTestFiles, locale: OperationalLocale): string {
  return locale === 'zh-TW'
...
function renderMergeScope(policy: ResolvedTestFiles, locale: OperationalLocale): string {
  if (locale === 'zh-TW') {
    return policy.architectureExemptions.length
...
function renderAgentPlacement(policy: ResolvedTestFiles, locale: OperationalLocale): string {
  if (locale === 'zh-TW') {
    return policy.architectureExemptions.length
...
export function renderEmptyTestFilesOperational(locale: OperationalLocale): string {
  return {
...
export function renderUnreachedTestFilesOperational(
  locale: OperationalLocale,
