...
const INSTRUCTIONS: Record<string, OperationalText> = {
  'review-retired-module-private': operationalText([
    ...
    '  imports), restate the private-layer intent with the current schema, even when the',
    '  config also calls a Blueprint preset: `allowedImporters` with `selfOnly`, or a folder',
    '  `layout` with an `entry`. Record anything the schema cannot express as an owner',
    '  decision in your report instead of inventing a rule.',
    '- If no owner source declares it, `module.private` came from a Blueprint 3.2 preset call,',
    '  and the 4.x preset carries its own governance for those layers. Confirm the listed',
    '  layers are still governed the way the owner intends.',
