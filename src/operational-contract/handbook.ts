...
export interface HandbookRuleFact {
  id: string;
  setting: RuleSetting;
  enforcement: 'docs' | 'inspect' | 'lint' | { unavailable: string };
}
...
export function renderArchitecture(architecture: ArchitectureDef, diagram: string): string {
...
export function renderRules(
  ...
  const rows = facts.map(({ id, setting, enforcement }) => {
    ...
    return [
      ...
      typeof enforcement === 'object'
        ? `nothing — ${enforcement.unavailable}`
        : HELD_BY[enforcement],
    ];
  });
