...
export function cleanupTargets(
  ...
  if (claudeDir.otherCommands > 0) {
    return `this playbook and \`${claudeDir.commandFile}\`, and nothing else: `
      + `\`.claude/commands/\` holds ${claudeDir.otherCommands} other command file(s) that `
      + 'are the owner\'s, so it and `.claude/` both stay.';
  }

  return [
    `this playbook, \`${claudeDir.commandFile}\`, and the now-empty`,
    ...(claudeDir.hadDir
      ? [
...
export function renderNextNote(next: boolean): string {
  ...
  return [
    ...
    '> File unit layout fits the route tree — '
...
export function renderHeader(
  nextNote: string,
  verdict: string,
  facts: { claudeDir: AuthoringClaudeDirFact; claudeLauncher: boolean },
): string {
  const { claudeDir, claudeLauncher } = facts;
  ...
  return [
    ...
    `> When you finish, delete ${cleanupTargets(claudeDir, claudeLauncher)} Then \`blueprint doctor\` has the final word: ${claudeLauncher
      ? 'it flags the two files as leftovers while they remain.'
      : 'it flags the playbook as an authoring leftover while it remains.'}${verdict}`,
  ].join('\n');
}
...
export function renderGoal(claudeLauncher: boolean): string {
  return [
    ...
    '',
    `**Early exit is a legitimate layer-first verdict.** On a repo below the brownfield threshold (${BROWNFIELD_MIN_FILES} source files) whose shape a framework preset already fits, the correct conclusion is to run \`npx blueprint init --preset --topology layer-first\`, wire its outputs, delete ${claudeLauncher ? 'this playbook and the command file' : 'this playbook'}, and stop. An explicitly selected module-first topology has no generic preset: continue the method and author its modules.`,
