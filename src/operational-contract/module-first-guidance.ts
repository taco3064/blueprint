...
export function renderModuleFirstGrowthGuidance(architecture: ArchitectureDef): string {
  ...
  return [
    ...
    '- **Materialize only after the boundary is chosen.** Use Module → Layer → Unit. Former LF '
      + '`containers` responsibilities live at each ordinary module root, not in an inner '
      + '`containers` layer. Route/page composition belongs in reserved `app` when applicable.',
    '- **Derive the graph from code.** Set `dependsOn` from real cross-module imports after the '
      + 'boundary is materialized. The temporary LF projection is not the active topology: never '
      + 'write LF config or invoke a topology transformation merely to perform this reasoning.',
  ].join('\n');
}
