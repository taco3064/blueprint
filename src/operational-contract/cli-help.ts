...
function renderInitHelp(brownfieldMinFiles: number): OperationalText {
  return operationalText([
    ...
    '',
    'With a blueprint.config.mjs, or on a proven-empty React/Vue project from canonical',
    'framework governance, generates:',
    '  · on a fresh layer-first scaffold, missing layer folders under the resolved',
    '    sourceRoot; on a fresh module-first scaffold, architecture.modules: []',
    '    selects the runway and no domain or repeated inner-layer folders are invented',
    ...
    'unmanaged tree as layer-first or module-first. A proven-empty React/Vue project',
    'may scaffold either topology directly from canonical governance. Brownfield',
    'module-first stays on semantic authoring. --preset remains an LF-only adoption',
    'method; do not combine it with --topology module-first.',
    ...
    '                          already complete. On a scaffold path NOTHING is',
    ...
    '                          explicit module-first, and every application with an',
    ...
    '                          Forces the PLAYBOOK, not a hand-authored config. For',
    '                          layer-first below the threshold, its verdict may be the',
    '                          early exit after survey has measured one application',
    '                          scope and that scope fits a preset; a multi-app workspace',
    '                          must choose --source-root first. The early exit ends by',
    '                          running --preset with an explicit layer-first topology.',
    '                          With module-first, the flag forces semantic domain',
    '                          authoring instead of the proven-empty modules: [] runway',
    '                          scaffold. Opposite of --preset; the two cannot be combined.',
    ...
    '                          On a proven-empty React/Vue project, module-first',
    '                          scaffolds canonical governance with modules: []. On',
    '                          brownfield source it uses semantic authoring; it cannot',
    '                          use --preset. layer-first remains compatible with',
    '                          --preset or --authoring. Valid configs are repository',
    '                          authority and must all resolve to one topology. On a',
    '                          configured repo, the opposite target starts the guarded',
    '                          Agent transformation for every adopted application; the',
    '                          same target repairs. Neither direction automatically',
    '                          moves or names code.',
    ...
    '  npx @kekkai/blueprint init --topology module-first # empty runway or semantic authoring',
    '  npx @kekkai/blueprint init --topology layer-first # flatten configured modules',
    '  npx @kekkai/blueprint init --topology module-first --agent claude # author brownfield MF',
  ]);
}
