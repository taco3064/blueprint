...
function renderInitHelp(brownfieldMinFiles: number): OperationalText {
  return operationalText([
    ...
    '  · on a fresh layer-first preset only, missing layer folders under the resolved',
    '    sourceRoot; a module-first runway creates no module folder, and existing code is',
    '    never padded with absent layers (no .gitkeep shells)',
    '  · eslint.config.mjs — structural rules + the embedded plugin',
    '  · the handbook at emit.handbook (default docs/architecture-handbook.md)',
    '  · only the Agent contracts selected by resolved emit.agents; fresh defaults',
    '    target Claude and AGENTS, while configured targets may use different paths',
    '  · compilerOptions.paths for the import alias (lossless edits only)',
    '',
    'Without any valid repository config, init requires an explicit --topology target.',
    'Surveyed folders and imports remain authoring evidence; they never classify an',
    'unmanaged tree as layer-first or module-first. --preset is only an LF adoption',
    'method, so first adoption uses --topology layer-first --preset.',
    'Greenfield and brownfield adopt differently. A proven-empty React/Vue application',
    '(0 source files) starts from the preset\'s complete canonical governance: layer-first',
    'scaffolds the preset, and module-first scaffolds it as a runway with `modules: []`',
    'and no invented domain module (Next.js module-first stays on authoring). Layer-first also',
    `scaffolds the preset below the brownfield threshold (${brownfieldMinFiles} source files). Other source`,
    'enters authoring, which translates house thresholds and ratchets pre-existing debt in the',
    'Blueprint baseline and ESLint suppressions: a safe floor, not the recommended ceiling, and',
    'tightening it toward the canonical preset later is the owner\'s call.',
    'An unresolved multi-app scope stops before every write. Once one config establishes',
    'repository topology, an unconfigured sibling inherits it and an opposite target',
    'is rejected. A configured opposite target starts one repository-wide transformation',
    'covering every adopted application after clean, recoverable Git preflight.',
    'Layer-first → module-first measures neutral domain candidates. Module-first →',
    'layer-first measures every structural destination and collision but requires the',
    'current config as authority. Neither direction moves or names source automatically.',
    'Next.js transformations preserve App Router; unsupported router changes are rejected.',
    '',
    ...
    '  --preset                Layer-first adoption method that skips authoring and',
    '                          scaffolds the framework preset. First adoption also',
    ...
    '                          a multi-app workspace first measures one application with',
    '                          `blueprint survey --source-root <path>`. The early exit',
    '                          ends by running --preset with an explicit layer-first',
    '                          topology. Four field agents reached for this flag and had to',
    '                          work that out from the run\'s output.',
    '                          Opposite of --preset; the two cannot be combined.',
    '  --topology layer-first|module-first',
    '                          Select the repository target for first adoption or a',
    '                          repository-wide change. Source shape is never authority.',
    '                          module-first authors modules from existing source, opens',
    '                          the canonical runway on a proven-empty React/Vue app, may',
    '                          be combined with --authoring, and cannot use --preset.',
    '                          layer-first remains compatible with --preset or',
    '                          --authoring. Valid configs are repository authority and',
    '                          must all resolve to one topology. On a configured repo,',
    '                          the opposite target starts the guarded Agent transformation',
    '                          for every adopted application; the same target repairs.',
    '                          Neither direction automatically moves or names code.',
    ...
    '  npx @kekkai/blueprint init --topology module-first # empty: runway; source: author',
    '  npx @kekkai/blueprint init --topology layer-first # flatten configured modules',
    '  npx @kekkai/blueprint init --topology layer-first --agent claude # author with agent',
  ]);
}

const SURVEY_HELP = operationalText([
...
const INSPECT_HELP = operationalText([
  ...
  'escapes, missing unit entries, selfOnly re-exports, and, when `rules.cycles`',
  'is declared with a tier other than `off`, import cycles,',
...
const IMPACT_HELP = operationalText([
...
const DEPS_HELP = operationalText([
  ...
  `  · ${renderTestFilesOperational('deps', 'en')}`,
...
const RULES_HELP = operationalText([
  ...
  '    cycles is diagnosed when declared and not off and `inspect` runs,',
  '    not continuously by lint)',
  '',
...
const DOCTOR_HELP = operationalText([
  ...
  '  · that reachable eslint leg passes live with the declared project-local ESLint;',
  '    unsafe or ambiguous argv skips instead of running a shell or another script',
  ...
  '    turns red and names what was lost. This check can SKIP: with no resolvable',
  '    merged config there is nothing to compare. An ignored or unmatched probe',
  '    is unverified; any actual rule loss in other probes still fails.',
  ...
  '    both the application and effective ESLint config directory are checked',
  '',
  'Flags:',
  '  --json                  Machine-readable output.',
  '',
...
}): Record<OperationalCommand, OperationalText> {
  return {
    init: renderInitHelp(facts.brownfieldMinFiles),
    survey: SURVEY_HELP,
