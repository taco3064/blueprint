...
function renderEarlyExitVerify(
  ...
  return [
    ...
    `   Either way, close this step by running the project's own lint once (\`${scriptCommand(pm, 'lint')}\`, or \`npx eslint .\` without a script): doctor now runs a safely replayable terminal ESLint leg, but never the other script segments, so the full command is still their proof.`,
