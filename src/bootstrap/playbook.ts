 ...
 */
export function renderHeader(nextNote: string, verdict: string): string {
  return [
...
export function renderGoal(): string {
  return [
    ...
    '**Those two branches are not a partition, and the third state is the common',
    'one: mentioned, but nowhere described as intent.** A box in a diagram and',
    'nothing else is exactly that — it says the clause is not stale without',
    'saying it is a seam, so neither branch fires. Do not force it into one.',
    'Keeping is already the default, so keep it and hand the owner the specific',
    'question: this layer is drawn in <file> and described nowhere, and the code',
    'it names lives at <path> instead. That sentence is worth more than a verdict',
    'you reached by picking the nearer branch — the owner knows which of the two',
    'the drawing meant, and you do not. Adding a state to the diagram-mention',
    'rule is what opened this gap: broadening "mentioned" narrowed the stale',
    'branch and left the middle unowned, so it is named rather than implied.',
    '',
...
export function renderMethod(hadClaudeDir: boolean): string {
  return [
    ...
    '   come out WORDED differently from the ones committed whenever a different',
    '   BUILD wrote them. Equal version strings do not rule that out: an unreleased',
    '   tree, a linked checkout and a git dependency each report the last release',
    '   while emitting later text. That is the improvement arriving, not drift and',
    '   not non-idempotency, and the check is the same either way — re-run init',
    '   twice and the second run is byte-identical to the first. Take the new',
    '   wording and say in your report that it changed. Never hand-revert generated',
    '   text toward what git happens to hold.',
