...
function renderCombinedEntry(): string {
  return [
    ...
    '     Check the emit points before merging, not after: `npx blueprint rules --json` lists the selectors under the entry each one belongs to — keyed `layer`, plus `module` wherever the config declares modules — so two importers show up as two, and two modules never share one string.',
...
function renderLintMerge(): string {
  return [
    ...
    '     Combine both option sets into ONE entry — blueprint\'s patterns and selectors plus your own (`npx blueprint rules --json` carries the exact selfOnly selectors as `jsLiteral` — paste that field, quotes included, never an emitLint dump. Take the row for the entry you are actually combining with: under `modules` each row is one (`module`, `layer`) pair and a neighbour\'s selector is a different string, so pasting the wrong row installs a rule that matches nothing and lint stays green over it. Its `note` says why the rendered `selectors` value is not the same string once it is inside JS source; that is a silent break too, so take the field that survives the paste rather than the one that reads more naturally).',
    '',
    renderCombinedEntry(),
...
export function renderMethod(claudeDir: ClaudeDirState): string {
  return [
    ...
    '5. **Choose module shape per layer.** High `index`-coverage child folders → `layout: \'folder\'` on that layer (`entry` defaults to `\'index\'`); plain files → the flat default (declare neither key — a layer that omits both resolves to `{ layout: \'flat\', entry: \'index\' }`).',
...
export function renderSemantics(): string {
  return [
    ...
    '  The fix for true hits is layout-dependent — flat: rewrite them as relative imports; folder: rewrite them as a relative import of the sibling\'s *entry* (`../Sibling`), never a deeper path into it (that one does trade the error for `relative-escape`).',
...
export function renderSchemaSketch(): string {
  return [
    ...
    '        // flat default ({ layout: \'flat\', entry: \'index\' }).',
    '        layout: \'folder\',',
    '        entry: \'index\',',
    '      },',
