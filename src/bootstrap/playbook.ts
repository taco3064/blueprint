...
export function renderGoal(): string {
  return [
    ...
    '**Runway comes in three shapes, and `inspect` names only the first.** An empty layer gets its note.',
    'An alias no import uses yet, and an `owns` entry for a package the repo has not installed (a preset\'s `hooks` owns `zustand` whether or not you use it), get none — nothing imports them, so there is nothing to count and no finding to raise.',
    'Both are still runway: they say where a thing goes if it arrives, ban nothing until then, and need no dependency added to justify them.',
    'Keep them on the same default as the layers, and know that these two are the runway you have to recognize yourself rather than read off a report.',
...
function renderIntentDocuments(): string {
  return [
    ...
    '   One exception, and it decides the harder cases: some clauses **cannot** be derived from the matrix at all — the shape of a selfOnly narrowing, the position of a layer holding no files, a permitted importer with zero edges today.',
    ...
    '   **A named import has its own evidence, and it is not the package row**: the survey lists every specifier that appears in exactly one folder, from a package that appears in several — `owns: [{ package: \'react\', imports: [\'createContext\'] }]` checks out when the pair is on that list.',
    '   Not on it is an answer too, not a dead end: either the specifier is spread across folders, and the clause does not hold today, or its package sits in one folder already and the package row above covers it.',
