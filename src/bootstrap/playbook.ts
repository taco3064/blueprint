...
function renderCombinedEntry(): string {
  return [
    ...
    '     Check the emit points before merging, not after: `npx blueprint rules --json` lists the selectors under the entry each one belongs to — so two importers show up as two, and two modules never share one string.',
    '     Its rows are told apart by `zone`, not by which keys they have: a `layer` row carries a `layer` key, while a module\'s own root (`root`) and the whole of a `layers: false` module (`module`) carry no `layer` and no selectors either — `allowedImporters` is a layer\'s field, so those two zones have none to emit. `module` is on every row wherever the config declares modules.',
...
function renderLintMerge(): string {
  return [
    '   - **Wire the lint.** If `eslint.config.blueprint.mjs` was written, merge it into the existing flat config: spread `...emitLint(blueprint, …)` — **carrying its options object over whole**: `stylistic`, `imports` and `projectRoot` always, plus `typescript: tseslint.plugin` on TypeScript projects.',
    ...
    '     **`projectRoot` is not a plugin and does not come from an import** — it is `dirname(fileURLToPath(import.meta.url))`, computed in the config file itself, so copy those two `node:` imports and the `const` across as well as the spread. It is the absolute path the rules measure your layers from; without it a checkout that sits inside a directory sharing a layer name (`components/`, `lib/`, `app/`) is read as living in that layer, and relative-import errors that `blueprint inspect` reports go silent in eslint. Doctor fails the merge-survival check when it is missing, so this one you will hear about.',
    ...
    '     Combine both option sets into ONE entry — blueprint\'s patterns and selectors plus your own (`npx blueprint rules --json` carries the exact selfOnly selectors as `jsLiteral` — paste that field, quotes included, never an emitLint dump. Take the row for the entry you are actually combining with: under `modules` every row carrying selectors is one (`module`, `layer`) pair — a module\'s own root and a `layers: false` module get a row of their own with none — and a neighbour\'s selector is a different string, so pasting the wrong row installs a rule that matches nothing and lint stays green over it. Its `note` says why the rendered `selectors` value is not the same string once it is inside JS source; that is a silent break too, so take the field that survives the paste rather than the one that reads more naturally).',
    '',
    renderCombinedEntry(),
...
export function renderRuleCatalog(): string {
  return [
    ...
    'When merging, collisions are decided by rule KEY, not by hit count — `blueprint rules --json` names every key the emitted config sets, and carries the exact selfOnly selector strings a fold needs, per entry rather than per rule: one row for each layer, or where modules are declared one per (module, layer) plus one per module zone — its own root, or the whole of a `layers: false` module.',
...
export function renderSchemaSketch(): string {
  return [
    ...
    '        name: \'features\',',
    '        does: \'…\',',
    '        // The module shape, declared per layer. Omitting both keys IS the',
    '        // file default ({ layout: \'file\', entry: \'index\' }).',
    '        layout: \'folder\',',
    '        entry: \'index\',',
    '      },',
