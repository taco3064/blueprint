...
describe('runInit · artifact hygiene', () => {
  it('regenerates its own eslint config in place instead of writing a reference', async () => {
    ...
    const again = await runLayerFirstInit(root, {
      install: false,
      log: silent,
  ...
  it('re-includes gitignored artifacts via negations instead of instructing (field '
    ...
    await runLayerFirstInit(root, {
      install: false, log: silent, loadConfig: async () => vuePreset(),
    });
...
describe('runInit · what a second run must not repeat', () => {
  ...
  it('stays quiet about the default target set once the config declares one', async () => {
    ...
    writePkg({ name: 'demo', dependencies: { vue: '^3' } });
    await runLayerFirstInit(root, { install: false, log: silent, agent: 'claude' });
