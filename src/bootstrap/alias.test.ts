...
describe('aliasActions · writing the paths into a tsconfig or jsconfig', () => {
  ...
  it('patches a parseable tsconfig.json in place', () => {
    const actions = aliasActions(
      state({
        tsconfigs: { 'tsconfig.json': '{}', 'tsconfig.app.json': null, 'jsconfig.json': null },
      }),
      ARCH,
    );
  ...
  it('follows a references shell to tsconfig.app.json', () => {
    ...
    const patched = aliasActions(
      state({
        tsconfigs: { 'tsconfig.json': shell, 'tsconfig.app.json': '{}', 'jsconfig.json': null },
      }),
      ARCH,
    );
  ...
  it('stays on the root when it is not a pure references shell', () => {
    const notShell = (root: string) =>
      aliasActions(
        state({
          tsconfigs: { 'tsconfig.json': root, 'tsconfig.app.json': '{}', 'jsconfig.json': null },
        }),
        ARCH,
      );
    ...
    expect(
      write(notShell('{"references": [], "compilerOptions": {}}'))?.path,
    ).toBe('tsconfig.json');
  ...
  it('ignores tsconfig.app.json when there is no root shell, and patches jsconfig', () => {
    const actions = aliasActions(
      state({
        tsconfigs: { 'tsconfig.json': null, 'tsconfig.app.json': null, 'jsconfig.json': '{}' },
      }),
      ARCH,
    );
  ...
  it('emits no write when the alias is already wired (idempotent)', () => {
    ...
    const actions = aliasActions(
      state({
        tsconfigs: { 'tsconfig.json': wired, 'tsconfig.app.json': null, 'jsconfig.json': null },
      }),
      ARCH,
    );
...
describe('aliasActions · the bundler instruct', () => {
  ...
  it('skips the bundler instruct when the vite config already quotes every alias (batch '
    + '10)', () => {
...
describe('aliasActions · root-level source (sourceRoot ".")', () => {
  it('wires the vite alias to the project root', () => {
    ...
    const vite = actions.find(
      (
        action,
      ): action is WriteAction => action.kind === 'write' && action.path === 'vite.config.ts',
    );
