...
export function authoringActions(survey: SurveyResult): Action[] {
  return [
    {
      kind: 'write',
      path: AUTHORING_FILE,
      content: authoringBrief(survey),
...
export function authoringBrief(survey: SurveyResult): string {
  return `# Blueprint authoring playbook
...
1. **Study the survey evidence below.** Every number is deterministic fact
   from this repo; do not re-derive it by grepping.
2. **Decide what is a layer.** Top-level folders under \`src/\` are candidates;
   root files are app wiring (never a layer). Test plumbing (\`test/\`,
   \`__tests__/\`) belongs in \`testFiles\`, not in \`layers\`.
3. **Infer the one-way flow.** Order layers so the *majority* direction of the
...
4. **Choose module shape per layer.** High \`index\`-coverage child folders →
   \`module: { layout: 'folder', entry: 'index' }\` on that layer; plain files
   → the flat default. Mixed repos usually need per-layer overrides.
5. **Assign ownership.** A package imported by exactly one folder (see the
   concentration list) is an \`owns\` candidate for that layer.
6. **Write the config** with \`defineBlueprint\` (schema sketch below).
7. **Validate — the loop that keeps you honest.** Run \`npx blueprint inspect\`.
...
8. **Finish.** Run \`npx blueprint init\`, then
