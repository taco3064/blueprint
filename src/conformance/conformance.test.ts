...
describe('greenfield scaffold — init alone completes (batches 1 & 4)', () => {
  it('init → inspect green → doctor reports every unavailable check', async () => {
    ...
    expect(doctor.output).toContain('⊘ Adoption unverified — 10 of 12 checks passed');
    expect(doctor.output).not.toContain('all 12 checks passed');
  });
});
...
describe('"complete" says what it leaves out (field runs #71–#73)', () => {
  it('names the uncommitted working tree under the banner', async () => {
    ...
    expect(doctor.code).toBe(0);
    expect(doctor.output).toContain('10 of 12 checks passed');
