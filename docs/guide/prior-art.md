...
| A CI workflow plus read-only `inspect` / `deps` runtimes | Nine finding kinds lint cannot see (undeclared folders, cycles, missing entries…) and a blast-radius query |
