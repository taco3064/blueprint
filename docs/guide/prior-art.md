...
entry-only unit boundaries, package and global ownership — compiled into an ESLint
...
`inspect` scans static syntax textually and uses an AST for dynamic `import()` targets
that reduce to a proven string ([the boundary](/guide/reference#how-the-import-graph-is-read));
runtime-dependent targets remain outside its graph, while dependency-cruiser resolves
imports more broadly. blueprint earns its place when the handbook and
