...
- Outcome — config authored from the survey; **246 real findings** locked as the baseline (including one genuine `services → types → resources → services` import cycle); per-layer unit layout (`resources` uses folder units). The legacy-eslintrc migration is surfaced as a decision, not forced.
...
- **Next.js**: `init` detects the route tree (`app/` and/or `pages/`, under
  ...
  the top layer, file unit layout, and **no `fetch` ownership** (server
