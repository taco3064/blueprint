...
框架種類由 `package.json` **自動偵測**（`--framework vue|react` 只在判不出來時才需要）；既有的 ESLint 組態**一律不覆蓋**（init 會改為提供合併指引）；重複執行 init 的結果具冪等性。
...
唯讀指令。掃描 `src/`、對照 blueprint 組態，列出所有違規與遷移建議；只要有 error 等級的違規，就以 exit code 1 結束。
