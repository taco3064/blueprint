...
單向分層流、folder unit 只能走公開入口、套件與全域物件的所有權 —— 轉譯成 ESLint flat config，plugin 內建。<br>
...
有一個差別值得白話講清楚：`inspect` 以文字掃描靜態語法，對可化約成確定字串的動態 `import()` 則解析 AST（[界線在這](/zh-TW/guide/reference#import-graph-是怎麼讀出來的)）；執行期才知道的目標仍不進圖，而 dependency-cruiser 解析 import 的範圍更廣。<br>
