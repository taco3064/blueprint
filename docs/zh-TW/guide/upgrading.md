...
有五則禁令訊息把 *module* 改成了 *unit*，產出的 Agent 守則也在同樣的位置改說 *unit*，<br>
而產出的架構手冊拿掉了一個原本就錯的說法 —— 「同層匯入一律禁止」。<br>
它從來就不是禁止的，而且只有一種合法寫法：用相對路徑指到兄弟單元的入口。<br>
你如果照那個說法把共用邏輯往下沉到更低的分層，那些你當初在閃避的匯入其實一直都是合法的。
