...
一份 `blueprint.config.mjs` 轉譯出四種產出物。本頁列出它們的實際樣貌 —— 內容取自在全新 Vue 專案上執行 `init` 的逐字結果。通則只有一條：**改 blueprint 組態，不要改產出物** —— 所有產出物都由組態重新生成，手動編輯的內容在設計上就會被蓋掉。
...
生成的 lint 組態刻意保持精簡：結構規則在跑 lint 時由 `emitLint(blueprint)` 展開，所以組態永遠不會跟 blueprint 脫節。這同時也是併入**既有** ESLint 組態的作法 —— 在自己的組態檔展開 `...emitLint(blueprint)` 就好：
...
給人閱讀的架構手冊：分層圖（mermaid）、職責表、模組形狀與匯入紀律 —— 跟 lint 規則出自同一份組態，所以不會彼此脫節。節錄如下：
