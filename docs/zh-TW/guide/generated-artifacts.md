...
給人閱讀的架構手冊：分層圖（mermaid）、職責表、單元形狀與匯入紀律 ——<br>
跟 lint 規則出自同一份 config，所以不會彼此脫節。節錄如下：

````md
## 架構

Code 單向流動：每一層只能匯入排在它後面的層。反向匯入一律禁止，而同層匯入只有一種合法寫法 —— 用相對路徑碰到鄰居的公開介面，絕不走別名。
...
**在 [`--structure modular`](/zh-TW/guide/structure) 之下，手冊開頭改成 `## Modules` 那一節** ——<br>
模組表、模組樹，以及外層的流向；<br>
上面那個 `## 架構` 小節這時描述的則是「每一個模組**裡面**」發生的事。<br>
產出的文件只帶它那個 repo 選定的那一種結構；兩棵樹都放在結構那一頁。
...
- **新增、搬移或重新命名任何檔案之前** —— 放在哪裡、單元形狀、專屬持有、命名、元件設計軸線、行為準則、作業守則：讀 [docs/architecture-handbook.md](docs/architecture-handbook.md)（由同一份 blueprint 生成 —— 永遠是最新的）。
...
- 硬性關卡（由機器強制，作用範圍是 layer glob 打到的檔案 —— 一層還沒有 code 就沒有東西會失敗，那是跑道，不是保護）：單向匯入、單元入口、專屬持有、相對路徑逃逸、`maxLines` = 400、`unusedVars`、`explicitAny`、`codeStyle`、`statementsPerLine`、`statementPadding`、`importBlock`、`fixtureImports`、`usePrefix`、`testFilename`、`deepWatch` 會讓專案的 lint 失敗；`cycles` 改由 `npx blueprint inspect --baseline` 把關，所以綠燈的 lint 對它什麼都沒說。lint 失敗時，去修結構 —— 永遠不要 `eslint-disable`，也不要把違規搬到隔壁檔案。
- 由你把關的部分：`~app/` 底下不得有未宣告的資料夾。新的頂層資料夾不會被任何 layer glob 打到，所以沒有任何 lint 規則看得到它，lint 全綠對它什麼都證明不了 —— 只有 `blueprint inspect --baseline` 會回報它（只對你新引入的東西變紅）。它的檢測項目會給兩個解法，而只有一個是你的：把 code 搬進既有分層的某個單元。如果架構真的長超過這份 config 了，那是擁有者的決定 —— 講出來然後停手；永遠不要自己宣告新的分層。
<!-- BLUEPRINT:END -->
```
