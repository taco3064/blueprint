...
`blueprint deps` 在動任何模組之前，先回答一個問題：**改這個模組，會波及誰？**它是唯讀指令，除了 blueprint config 本身不需要任何額外設定，也不會寫入任何檔案。

它與 [`blueprint inspect`](/zh-TW/guide/getting-started#既有專案-——-blueprint-inspect) 的分工：`inspect` 負責**裁決**架構（違規、循環相依、exit code 1），`deps` 只負責**描述** —— 逐模組列出被誰引用、引用了誰，不做任何判定。
...
查詢不存在的模組時，以 exit code 1 結束，並提示可以跑排行榜列出所有模組；查詢成功則以 exit code 0 結束。

## 查詢粒度 —— 由 `module.layout` 決定

每個查詢結果的單位是**模組**，而模組的界定方式取決於 Blueprint config 中的 [`module.layout`](/zh-TW/api/interfaces/ModuleDef)（可透過 `layer.module` 逐層覆寫）：
...
- **測試檔案一律排除**（`architecture.testFiles`）—— 測試對模組的匯入不算進影響範圍，跟 lint 側的行為一致。
...
手寫、沒包 `defineBlueprint` 的 `blueprint.config.mjs`，載入時一樣會跑完整驗證。結構性錯誤會立刻以精確訊息回報，而不是在指令跑到一半時炸出一個難以定位的例外：
