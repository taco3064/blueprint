...
- **`init`** —— `--agent claude|codex`（啟動編寫／轉換用的 Agent CLI）· `--topology layer-first|module-first`（首次 repository adoption 必填；source shape 只作為證據，有效 config 共同建立唯一 repository topology；configured opposite target 會啟動受 Git preflight 保護的 repository-wide transformation）· `--preset`（layer-first adoption 方法；首次導入需明確 LF，新 sibling 可繼承 repository LF，repository MF 與已有 authored local config 都會拒絕）· `--authoring`（即使小 repo 也強制產 playbook；與 `--preset` 相反）· `--framework vue|react` · `--no-install` · `--dry-run`

layer-first → module-first 轉換要求：Git worktree 乾淨、存在可復原的 committed `HEAD`，且每個已導入 application 的轉換前 inspection 都能提供可靠證據。repository-root playbook 會為每個 application 分開帶入 container／page domain 候選 closure 與 page／App Router composition closure，並列出使用 inspect／deps canonical identity 的 edge、無法匹配的 alias-like import、有限的 relative path 結構證據、Git 搬移規則、cutover gate 與 baseline review；精確 relative file resolution 與 runtime-dependent import 仍是明確限制。domain 命名、ownership、merge／split、中立模組抽取、cycle 與 collision 則由 Agent 判斷，再以 `git mv` 搬移並改寫 import。Next.js App Router 保留原本的實體 `app/**`；Pages Router 因為需要框架 router migration，會在寫入前拒絕。
