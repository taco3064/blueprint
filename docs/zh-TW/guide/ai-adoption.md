...
**蒐證** —— 頂層形狀、資料夾結構、匯入關係矩陣、單元形狀、套件集中度
- 執行者 — 決定性程序
- 工具 — `blueprint survey`
...
- **頂層形狀判定** —— `flat` 或 `modular`，並附上判定的依據（`--json` 的 `shape` 鍵、人類可讀輸出的 `Top-level shape:`）。<br>
  這就是 [`--structure`](/zh-TW/guide/structure) 要的那個答案；證據不足以判定時，它會印出 `COULD NOT TELL`，不會用猜的
- 頂層資料夾清單與**單元形狀證據**（`index` 檔覆蓋率、巢狀深度 —— 作為判斷 `folder` 或 `file` 佈局的依據）。<br>
  判定為 `modular` 時，同一批列讀的是各模組的子項
