...
`--authoring` 保證即使在小 repo 上也會產出 playbook（純 `init` 在檔數低於門檻時會改建 preset、不產 playbook）。三條驗收各自對應實測中出現過的未完成狀態：整合只做一半、檢核沒跑完、把還債混進導入。有兩條在特定 repo 上會「空泛地成立」而且這樣就對了：沒有測試的 repo，「原有測試都過」直接成立、不用去補 test runner；零債的 repo，鎖帳指令直接跳過、不用為了儀式跑一遍。全新專案不需要這段 —— `init` 一個指令就完成；`init` 跑過之後也可以在 Claude Code 直接輸入 `/blueprint-author`。
