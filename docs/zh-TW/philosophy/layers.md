...
每個狀態儲存庫都該有一個唯一擁有它的 hook 資料夾，這個 hook 就是它對外的介面；其他功能一律透過它讀取。<br>
至於 `utils/`，它就是個沒有內聚性的雜物間：<br>
任何「看起來很通用」的東西都會被丟進去、越長越大，最後變成所有資料夾都依賴的耦合點。<br>
純函式該照「誰在用」來歸屬：<br>
只有單一資料夾在用，就當那個資料夾的私有檔案；<br>
跨資料夾要共用，就開一個有名字、依領域切分的獨立資料夾，讓它「掙到一個名字」。
...
└─ Dropdown/
   ├─ index        ← 對外唯一入口（公開）
   ├─ Dropdown     ← 實作本體，檔名即資料夾名（不命名為 Component）
   ├─ hooks        ← 私有
   ├─ styles       ← 私有
   └─ types        ← 私有
```

- `index` 是資料夾對外的**門面**，外面只認得這個入口
- 私有子元件就放在資料夾裡（例如 container 底下的 `ProfileTab`）——<br>
