...
如果生命週期狀態損壞、包含無法驗證的資料，或曾經進過 Git 歷史卻不見了，Blueprint 會停止需要這份狀態的操作，不會自行重建或猜測歷史；讀不到 Git 歷史時也一樣。請先從版本控制或其他可信來源還原，再重新執行原本的指令。

如果這個檔案從來沒有進過 Git 歷史，就不會被當成遺失：沒有證據能證明 lifecycle 曾經成立，所以這個專案會被當成生命週期狀態功能加入之前就已導入的專案。Git 分不出「套件被直接升級」和「導入後從沒提交這個檔案」這兩種情況，所以請把這個檔案提交進版本控制，讓它的歷史紀錄保持有效。
