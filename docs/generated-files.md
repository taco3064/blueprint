...
adoption that has not finished — it failed part-way, deferred a required install with
`--no-install`, or handed off to an authoring playbook — keeps the records for what it wrote,
without claiming a completed lifecycle; the `init` run that completes adoption establishes the
checkpoint. `upgrade` records a pending upgrade before changing anything and moves the checkpoint
only after verification, and only while the file still records that pending upgrade. `remove`
...
leaves the previous state intact. A leftover `.tmp` file is never read as lifecycle state; the next
write replaces it, and `remove` deletes it.
...
`destinations` contains exactly the `members` destination **file paths**, never module or unit
directories. For example, use `["src/app/Game.tsx"]`, not `["src/app"]`. Every origin member must map exactly once to a globally unique destination. Blueprint compares its Git
...
them while adoption remains unfinished. For legacy ESLint, retain `eslint.config.blueprint.mjs`
until the owner chooses and completes flat-config migration. Doctor identifies that intended
reference without asking for premature deletion; the separate unwired ESLint check remains incomplete.
...
not point at files that no longer exist. It checks both the application directory and the effective
ESLint config directory, resolving each entry relative to its own ledger.
