...
- **Touched in a way that breaks the citation's premise, or a citation surfaces a product decision the plan never actually made** — the module was removed, a consumer's real behavior contradicts what the plan assumed, an ambiguity the three headings' presence didn't catch — **stop before the next commit.** Comment it on the ticket, naming the citation, what it now says, and what it contradicts. **Deriving the missing decision yourself from the nearest analogue is exactly the failure this gate exists to prevent** — a ticket that reaches deliver-ticket is supposed to carry no open product decision, and a decided shape with a gap in it is a direction wearing a shape's clothes, not a shape with one flaw.
  ...
  **The one exemption is a worked example that is wrong while what to build is not.** A plan's illustration — a quoted output, a sample name, a path that does not resolve — can be false with the goal, the stage boundaries and every acceptance criterion still standing, and then neither of `start-or-resume.md`'s two branches has anything to revise. **That one is a shortfall, closed in the loop, and the review side charges it nothing** — a wrong worked example with the tree clean of it is what `review-stage`'s FOLLOW-UP carve-out is for, so it moves no verdict and spends no round. Stopping the delivery instead spends the owner's turn on a decision whose only answer is *the example is wrong, carry on* — the whole price of the class, on the remedy that buys least. **The test is whether the wrong text changes what the stage has to make true**, never whether the ticket contains something false.
...
- **A claim this skill wrote that is later withdrawn, superseded or overtaken is marked at the claim itself, not only at what replaced it.** **The same three modes, in this side's own instances** — a shortfall ruled inside and later ruled outside, a measured claim a later stage falsifies, a disposition the owner later overturns: **edit the original comment to say which of the three happened, where the role word already is, and to point at what replaced it, and leave the rest of its body intact**, because the reasoning that was wrong is what stops it being re-derived. **The mode and the placement are the obligation and not decoration**: a mark that does not say which of the three happened leaves a reader guessing whether the author withdrew the claim or a later fact overtook it, and one that sits anywhere but where the role word already is fails the test the role word passes — it has to read as marked without the comment being opened. `shape-ticket`'s side of this convention states both, and the two sides move together or not at all. Find it the way `deliver-a-stage.md`'s *The review state goes on the ticket at the dispatch, not at the first BLOCKED* finds its own — through `gh api`, never `--edit-last`, which targets the latest comment and on this ticket something will have landed since. **The incident is the shaper's** — a fix-budget ruling refused with evidence and withdrawn two minutes later in a separate comment, the original left standing unmarked — **and the exposure is this side's too**: a reader who stops at a stage comment is reading a claim that nothing on that comment says was overtaken.
- **A cross-reference is written after its target exists, never before.** A citation **asserts that the thing it points at is there**, so a URL, a comment number or a file path written ahead of the write it names is false at the exact moment it is most likely to be read, which is while the reader is following it. **An exit code reports that a command ran; a pipeline's does not report that it finished; a diff reports that something changed, not that the intended thing did; and a citation reports nothing at all — it asserts.** The first three are checked by reading the changed line back. **The fourth has nothing to read back, and is checked by ordering: create, then cite.** **Both instances are this skill's own, and it disclosed both**: a review-state comment carrying an escalation's URL before the escalation was posted — the number wrong, corrected after — and a dispatch naming a commit-message path before the message existed, **which claimed the draft was already there to open**. **Both said *the thing is here*; neither told anybody to put it there**, and the second one could not have: the draft commit message is this session's own artifact, per *The commit is yours*, so no dispatched agent was ever going to create it, and the path pointed at nothing until this session wrote one. **That is what keeps it an instance** rather than an instruction to write, and this reading of it is delivery's own — `shape-ticket`'s copy of this rule narrates the same two instances in its own words and records that its own run has none. It is also what makes the withdrawal rule executable: post the replacement, then edit the original to point at it.
  - **An instruction to write somewhere is not a citation, and this rule does not reach one.** A dispatch naming the file its agent is to create, and the review-state comment recording that path at the dispatch, both name something that is not there yet — and neither says it is there: they say where to put it, and the agent's own writing is what a reader eventually follows the path to. **The test is what the sentence claims, not whether the target exists when it is written**: a pointer saying *the thing is here* is false until the write lands, and one saying *put the thing here* is what makes the write land at all. So `deliver-a-stage.md`'s *A dispatched agent's report is written before it is returned* names a path ahead of its file on purpose and is no breach of this — and it is the reason the rule needs saying out loud, since a reader meeting that page next to this one otherwise reads it as one. **It does not reach the two disclosed instances**: the exemption is for a file the dispatched agent is the one to create, and neither of those is that — the commit-message draft was the dispatching session's own to write, and the escalation URL pointed at a comment nobody had posted — so both stay recorded as instances, and a reader meeting the rule and its instances together has nothing left to reconcile. **This clause is delivery's own and moves nothing held in two places**: `shape-ticket` names no report file and records no handback path, so there is nothing on that side for it to have a counterpart in. **It does dispatch agents** — `create-the-ticket.md`'s *Independent feasibility check*, and `deliver-ticket` itself under that same page's *The owner's word starts delivery* — and neither returns a handback of the kind this clause is about: `shape-ticket`'s own *A progress report reads the GitHub record, never the agent's last message* points that side at the ticket rather than at a file. And *create, then cite* itself is unchanged on both sides of the seam.
