...
**A correction to it does not get a stage either, and the exemption above is worth nothing without this.** A shortfall against the note's wording that arrives as a stage brings the whole machinery with it — an implementer, a reviewer, a fix budget, a re-review per round — on an artifact nothing gates before the release tag. #380 did that twice: two stages whose entire staged diff was this file plus code comments, four fix rounds and two escalations between them, and no line of code in either. **A shortfall whose staged diff touches no file under `src/` is repaired in the commit that closes it and verified by the release-note reader**, which reads this file in step 5 regardless. `git diff --cached --name-only` is what tells the two apart.
...
- **Fresh context, latest branch, and unable to write anything but its own report.** A reader that can edit starts explaining instead of reporting — so nothing in either tree, nothing staged, nothing on the ticket. **The one file it writes is the report file this step names for it**, below, which is outside both worktrees and is the only way its findings survive the trip back.
...
- **Build the before-tree and the after-tree once, yourself, and hand every reader the same two paths.** Each reader told to run needs both, and each one building its own is the same bundle produced once per dimension: #380's five readers built two trees ten times, the single largest concentration of cost on that ticket. **Hand over the paths and the commit each was archived from, and ask each reader to verify that identity rather than reproduce the build.** One that distrusts the trees may still build its own and say so in its report — what it may not do is build one by default.
...
- **Name the file each reader writes its report to, and read the file rather than the message.** `deliver-a-stage.md`'s *A dispatched agent's report is written before it is returned* holds this for the two kinds that page dispatches and says outright that *"a third kind of handback is a new ticket, not a wider reading of this one"*. **These readers are that third kind. This is the rule it sent them for**, written on the page that dispatches them rather than by stretching the one that does not. The dispatch names the path, the reader creates it before returning, and hands the path back with the verdict.
- **The failure it prevents is not the reader dying — it is the report arriving somewhere you are not.** #387 ran four readers whose reports reached the session watching the delivery instead of the session that dispatched them, so **nothing you could read existed** and the completion test could not have finished. It was recovered only by resuming each reader to transcribe its own finished report — a recovery that works exactly once and only while the readers are still alive. **A report in a message is addressed to a mailbox; a report in a file is addressed to whoever holds the path.** *(The reports reached that session's transcript and the ticket carries the recovery, not the incident; the readers' own scratch directories are machine-local and outlast neither.)*
- **This rule is half of a pair and the other half is `review-stage`'s.** Its `SKILL.md` states the carve-out from the reader's side and `write-the-verdict.md` states that no caller is exempt from it; **all three move together or not at all.** They have to, because the reader loads `review-stage` and never loads this page, so a change made only here reaches nobody it governs — **which is exactly how this rule was first written wrong**, with these bullets added and both of `review-stage`'s sentences left saying the opposite.
