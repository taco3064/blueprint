...
**Before merging, confirm the branch is up to date with `origin/main` — don't assume the platform enforced it for you.** `git merge-base --is-ancestor origin/main <branch>` (exit 0 means yes) or `gh pr view --json mergeStateStatus` says whether every commit currently on `main` is already an ancestor of this branch. If it isn't, update the branch (merge or rebase `origin/main` in) and wait for CI to go green on the resulting `HEAD` — a new commit is a new candidate, and the rule above about reading CI against current `HEAD` applies to it too.
...
**Then confirm identity, not correctness.** Because the branch was just confirmed up to date with `main`, the tree GitHub produces — a squash commit's diff applied to the current tip, or a rebase's replay onto it — has to equal `<verified-sha>`'s own tree exactly. `git diff <merged-sha> <verified-sha>` empty is the whole check, and it's cheap for that reason, not because it's being skipped.
