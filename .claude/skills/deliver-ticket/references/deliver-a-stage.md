...
**Two shape `VOID`s on one stage is the limit — counted across both checkpoints and kept on the ticket, not in your context.** The part-two gate and this one spend the same budget, because they are the same failure appearing at two moments. A third ends the stage the way an exhausted fix budget does — park the work, comment, stop. A reviewer that cannot produce its own required output is not one whose PASS means anything, and re-dispatching a fourth time spends the owner's evening proving that.
...
- **VOID** → nothing was reviewed. **Two origins, one handling.** The reviewer raises it for a condition in its packet — insufficient, a contaminated pass one, a moved tree, a requirement that moved under it — and you fix that condition and dispatch again. **You raise it for the report's shape** (above), and *which* block decides the remedy: a pass-one block missing at the part-two gate goes back as a whole first pass; one that moved by the final report goes to a fresh reviewer; a missing `Claim audit` is the only one re-dispatched for the block alone. **A VOID does not consume a fix round** either way, and neither does a finding the reviewer withdraws.
...
- **The round number against the budget**, and — **counted separately, never folded into it** — **the shape `VOID` count, 0 to 2.** A shape `VOID` costs no fix round by construction, so a single number cannot carry both; and a resumed session that cannot read this one has a limit that silently resets every time a session dies, which is the same defect the round count is on the ticket to avoid. Write it before the re-dispatch, not after.
