...
**And a requirement that moved is not a protocol failure at all.** The owner edited the issue: the old verdict is void because the question changed, not because anything failed. It costs neither budget, and the stage restarts from part one against the current text. **But a second movement on one stage stops** — the same *twice is evidence*, from the other direction. Chasing a requirement still being written is a loop with an external motor, and the useful response is to say so and wait, not to spend the night re-reviewing drafts.
...
**Two fix rounds is the budget.** When the second re-review still returns BLOCKED, **the stage escalates and nothing commits** — and this is the exit *The protocol's own budget* uses too, so **the comment says which budget ran out**: they fail for different reasons and what the owner does next differs. An exhausted fix budget is evidence about the code, the plan or the ticket; an exhausted protocol budget is evidence about the loop, and re-running it unchanged reproduces it.
