...
**Four causes increment it, and they share the one property that makes retrying sensible: the failure is ours, and the same dispatch run again could succeed.**
...
- **`tree-fixer`** — the commit gate's fixer moved the tree the verdict was on; *Restoring an unpushed commit to the review target* is the remedy.
...
**Three things never increment it, because retrying is the wrong response to all three.** They go straight to the owner, through the same escalation exit an exhausted fix budget uses:
...
- **A tree that moved and you cannot say what moved it.** A known mover has a known answer, which is why `tree-fixer` is retryable. An unknown one means **you do not know what a re-dispatch would be reviewing**, and dispatching anyway is how a review of something nobody identified gets recorded as a review of the stage.
...
**Two fix rounds is the budget.** When the second re-review still returns BLOCKED, **the stage escalates and nothing commits** — and this is the exit *The protocol's own budget* uses too, so **the comment says what ended the stage** — the three causes fail for different reasons and what the owner does next differs. An exhausted **fix** budget is evidence about the code, the plan or the ticket. An exhausted **protocol** budget is evidence about the loop, and re-running it unchanged reproduces it. **A requirement that moved twice is neither**: no budget was spent, the work is intact, and the only thing owed is a requirement that holds still.
