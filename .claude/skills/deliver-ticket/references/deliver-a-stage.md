...
**Then read the report's shape before its verdict word.** A PASS is an assertion *about the report's own blocks* — that the scope was stated, the axes listed, every pair they make accounted for, every probe class named, every claim placed. **With a block missing, the word `PASS` is not a verdict you can act on**, and branching straight to the commit is what makes `review-stage`'s *PASS asserts…* advice rather than a gate. Four things, checked mechanically:

- **`Claimed scope`** — present, non-empty, and in behavioural terms. A file list there is an absent scope, because the line every FOLLOW-UP is judged against has to say what the stage asserts is *true*.
- **`Axes in play`** with their values, and **`Dimensions crossed` accounting for every pair those axes make** — probed, or ruled out with a reason. A pair on neither list is exactly the ambiguity the block exists to remove.
- **`Verified`** naming every probe class — run, or ruled not triggered with the reason.
- **`Claim audit`** placing every claim from the report you sent in part two.

**Any one of them missing is `VOID`** — send it back naming the block, and **it consumes no fix round**: the work is not what failed and the tree never moved. **You are checking the artifacts exist, not grading them.** A dispatcher that starts deciding whether a crossing was the *right* crossing, or a scope drawn correctly, has become the second reviewer this gate exists to not be — and it would be doing it holding the implementer's report, which is the anchoring the two passes were built to keep out.

**Two shape `VOID`s on one stage is the limit.** A third incomplete report ends the stage the way an exhausted budget does — park the work, comment, stop. A reviewer that cannot produce its own required output is not one whose PASS means anything, and re-dispatching a fourth time spends the owner's evening proving that.
...
- **VOID** → nothing was reviewed. **Two origins, one handling.** The reviewer raises it for a condition in its packet — insufficient, a contaminated pass one, a moved tree, a requirement that moved under it — and you fix that condition and dispatch again. **You raise it for a missing report block** (above), and that one is re-dispatched for the block alone: the packet was sound and rebuilding it would restart a review that already happened. **A VOID does not consume a fix round** either way, and neither does a finding the reviewer withdraws.
