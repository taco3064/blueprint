...
Four ways a question gets manufactured, all four from that run and the delivery that followed it:
...
- **A shortfall the ticket's own boundary answers is not an escalation either.** `deliver-ticket`'s completion test hands its open shortfalls to this side, and the bar in this section decides them the way it decides everything else. Three of the four relayed on #379 were answerable from that issue's own *Boundary* and *Out of scope* — text this skill had written itself — and the fourth asked whether `src/bootstrap/ignored.ts` reaching into `inspect` is a deviation, when `src/inspect/index.ts` answers it at the export being asked about: *"reaching them by deep path is what the entry rule bans"*. **What is the owner's here is the ruling, never the classification.** `finish-the-ticket.md` closes a ticket only when no shortfall is open, and *"the ones ruled outside the ticket by the owner are the only exception"* — that requirement is `deliver-ticket`'s, nothing here moves it, and what this skill hands him is the disposition it has already decided together with the basis for it, not the question of what the disposition should be. *The floor — is it related to what this ticket is fixing* on this page states the disposal that hand-over carries, and states which hand-over it is.
...
**A defect the work happened to sit next to is not** — however true, however measured, however adopter-facing. **It is recorded in the issue, and the ticket ships.** Not a stage, not a follow-up, not a deferral, not a revision pass, not a decision parked so the owner can weigh it at some later point: the record is the whole disposal, and there is nothing else on offer. **A finding enters `revise-an-in-flight-ticket.md` through this test, not beside it** — that page's *The floor is checked before the new-evidence case of that trigger is* states the same from its own side, and scopes it: this is a test on findings rather than on edits, so new evidence reaches a revision pass only by clearing this floor first. **An edit that carries no finding is not gated here at all** — a body correction, a re-worded decision, a fixed citation, a label: each of them enters that page for the announcement rule *When it lands, and who hears about it* holds, which binds every edit and not only the ones that move a requirement.

**One hand-over survives this floor, because it ends a ticket rather than postponing one.** `deliver-ticket`'s `finish-the-ticket.md` closes a delivery only when no shortfall is open, and *"the ones ruled outside the ticket by the owner are the only exception"* — so a shortfall this test resolves to *recorded, ship* still needs that ruling before the ticket can finish. **Asking for it is not a deferral and is not what the single disposal refuses**: what is refused is parking the decision for later; what is required is carrying it, already decided and with its basis, to the one person whose ruling closes that gate. **The incident is this skill's own** — #385's floor as first landed named that hand-over among the things it bans, which left a shaper holding a disposal for every shortfall and no route to the step that closes a delivery, and that is the step #385's own completion test stopped on. The requirement that the ruling be the owner's is `finish-the-ticket.md`'s and nothing here moves it; the classification stays this skill's, per *Classify before you ask* on this page.
