...
**One of the two documents part two hands you is not in the tree you reviewed.** The draft commit message is nowhere in the diff of record — `git diff origin/main...HEAD` — so correcting it moves nothing any probe was run against. **So when a finding's only subject is that draft, say so in the finding** — the dispatcher cannot otherwise tell it from a finding on the tree (`deliver-a-stage.md`, *Reading the verdict*).
