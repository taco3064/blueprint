...
- **The base**: the branch's `HEAD` SHA, the worktree path, and the fact that the change under review is *staged and uncommitted* there. **That is the only target a stage review has** — there is no mode of this skill that reviews a commit that already exists. A dispatcher holding an unreviewed local commit restores it to a staged diff first (`git reset --soft`), and one holding an unreviewed *pushed* commit stops for the owner rather than inventing a review after the fact; `deliver-ticket`'s `start-or-resume.md` owns both. **A dispatch pointed at a commit SHA is a packet you cannot review** — `VOID`, *Insufficient*, below.
