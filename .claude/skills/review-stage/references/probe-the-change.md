...
So before probing, **write down the axes this change actually puts in play, with their values**. For the module axis that came to: module shape (layered / `layers: false`), import site (module root / layer entry / inside a layer), spelling (relative / alias), permission (allowed / forbidden), source root (`src` / `.` / a nested path).
