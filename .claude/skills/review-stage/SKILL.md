...
| Reference | Trigger |
|---|---|
| [`rebuild-the-picture.md`](./references/rebuild-the-picture.md) | The Review Packet has arrived and you have not run anything yet. Pass one — what you read, in what order, the six questions you answer yourself, and what a packet that cannot be reviewed looks like. |
...
**No edit, no commit, no push, no `git add`, no branch, no stash, no reset, no `--fix`.** No GitHub issue, no comment on the ticket, no PR, no label — the dispatcher owns every word that reaches the ticket, and a reviewer that comments there has published an unverified finding under the delivery's name. **Your report goes to whoever dispatched you and to the one file that dispatch named — when it named one — and nowhere else.**

**Write it there before you return it — the pass-one handback first and the final report over it — and hand the path back each time**, with the `pass one — not yet computed` line and then with the verdict line. **One file, rewritten as the review advances**: the four pass-one blocks are frozen on the ticket by the dispatcher before part two goes out, so the file's job is to hold whatever you last wrote for whoever has to read it without you. **A report that exists only in the message carrying it dies with whoever was going to receive it**, and #385's stage IV is the case: a reviewer raised a REQUIRED and was killed between part one and part two, its handback never reached the dispatcher, its replacement's independent pass one did not find the same thing, and a completion-test reader found it again two stages later. **This changes nothing else about the report** — not what is in it, not who reads it, not when either pass is written. **And it reaches only a dispatch that named a file.** deliver-ticket's completion test **now names one** — `finish-the-ticket.md`'s *Name the file each reader writes its report to* — so its readers write their report before returning it, exactly as you do, and the *"unable to write"* in that page is scoped to the trees and the ticket rather than to the report. **That carve-out used to run the other way and the reason it changed is measured**: #387's four completion readers returned their reports as messages to a session that was not the dispatcher's, so nothing readable existed and the step could not have finished. **It is still named here rather than only there**, because a page about what a reader may write is the wrong one to leave a caller holding an instruction its own dispatcher forbids — and that cuts both ways: leaving this paragraph saying *no file* while that page names one is the same defect with the sign flipped. **A file is not a comment**: it reaches one dispatcher, it publishes nothing, and the dispatcher still owns every word that lands on the ticket.

**Read-only includes the tree you are reviewing.** The change under review is *committed* — the assembled ticket at the branch tip — so nothing you type destroys it; what a write does instead is move the worktree off the tree the packet names. A formatter, an autofix, a "while I was here" one-liner does not merely break this rule; it turns your verdict into a verdict on a tree that no longer exists, and makes every later reading of that tree ambiguous between the implementer's work and yours.

**So the dispatcher checks rather than trusting this page**: it takes the code hash — the reviewed commit's own tree, `git rev-parse <sha>^{tree}` — and `git status --porcelain` before dispatching you, and again when your report lands. **A tree that moved voids the review** — the verdict is discarded whatever it said, because there is no longer any way to know which change it was a verdict on.

**Anything you need to write goes outside the worktree.** A fixture repo, a rendered artifact, a probe script, a byte baseline — the session's scratchpad directory or a `mktemp -d`, never the tree, not even under a gitignored path. `.claude/docs/verification-layers.md` calls the byte baseline scaffolding rather than a contract, and this is where that bites: scaffolding left in the tree is indistinguishable from the stage having added it. **The report file is in that same category with one difference: the dispatch chooses it rather than you**, and it is named there so that somebody who is not you can find what you wrote.
...
- **Pass two — audit.** Once your pass-one findings are written and handed back, the dispatcher sends the implementer's report and commit message for every stage the ticket landed, verbatim. Now each claim in every one of them gets classified and checked against what you already found. `write-the-verdict.md`.
...
- **REQUIRED** — the direction holds, but this stage's own acceptance is not met: a criterion with no executed check, a missing branch in the tests, a fix that treated the instance instead of the mechanism. → **BLOCKED**.
  - **Every REQUIRED names who acts wrongly because of it.** Not what is imprecise, inconsistent or inelegant — **who follows this and does the wrong thing**: a session that takes the wrong branch, a reader who applies a rule the other way, an adopter whose build differs. **If you cannot name that actor and the wrong act, it is FOLLOW-UP**, however true the finding is. A count that is off, a lead sentence that disagrees with its own paragraph's body, a price named with the wrong word — each is real and each is FOLLOW-UP unless somebody would act on it and be wrong.
  - **This bites hardest when the deliverable is prose**, because then every sentence is "the tree" and every imprecision looks like a defect in the deliverable. It is the same test `write-the-verdict.md` applies to the issue's own prose and to a commit message — *name who is harmed* — and it applies here too. **This repo priced the alternative**: one ticket whose deliverable was a set of rule files spent a day of review rounds on sentences that decided the same cases either way. **Strictness is for the code and for what a user sees.**
- **FOLLOW-UP** — real, addressable, and genuinely not this stage's: it predates the change, the change neither touched nor widened nor claimed to fix it, and no acceptance criterion here depends on it. → **does not move the verdict.** This severity is what keeps a reviewer from expanding without limit, and it exists because deliver-ticket's *you do not create tickets* and *you do not absorb it either* are both in force above you — a FOLLOW-UP raised as REQUIRED is how a diff quietly grows a second reason for existing.
  - **Three routes reach FOLLOW-UP without those three conditions — two named subjects and one test — and this is the first subject: a defect whose only subject is the issue's own illustrative prose** — a worked example naming the wrong artifact, a count, a sentence characterising something the criterion cites — **and you say in the finding that it is this case**. It fails the third condition by construction, since it lives *inside* a criterion, and it would otherwise be forced to REQUIRED and stop the tree on a sentence the tree does not contain. **The carve-out is narrow in two ways, and both of them bound it here rather than on the other page.** ***Only* the issue**: the same sentence copied into the tree under review has the tree as a subject too and is REQUIRED on the ordinary grounds. ***Only* illustrative**: a criterion that **cannot be decided as written**, or that **describes the deliverable wrongly**, stays REQUIRED — neither is illustrative, and each changes what gets built. `write-the-verdict.md`'s *A third subject* carries the full statement; this is here because the classification is usually reachable in **pass one**, and that page is not open yet.

    **The second subject is a defect whose only subject is a commit message this ticket landed**, and `write-the-verdict.md` states it in full where it states this one. **It escapes by a different condition** — the first fails the third, since it lives inside a criterion; this one fails the first, because a message written for this change does not predate it — **which is why they are two and not one.** **Pass one cannot reach it at all**, the messages not being handed over until part two.

    **The third is not a subject but a test: the REQUIRED rung above.** A finding whose actor cannot be named arrives here whatever its subject, **including text this stage itself authored** — so it is the one of the three that reaches the tree. **The other two each escape by failing one named condition; this one is not routed by the conditions at all** — the rung above sends it here whatever they say, and a defect in text this stage authored fails the first two of them anyway. **Counting it is not bookkeeping**: a reviewer holding both rungs and a count that stopped at two meets its own actor-less finding, finds it is neither named subject, falls back to REQUIRED, and blocks the stage on exactly the sentence the rung above was written to stop blocking on. **The count is stated here and nowhere else**: this page is the one always in force, a reader holding it has to know where the list closes, and a count kept on both pages is a count that drifts.

**"An earlier stage introduced it" is not one of those three conditions, and a stage whose own job is verification is where that matters.** Its diff adds tests; what it *claims* is that the assembled behaviour holds — so **the behaviour is its scope**, and a reproducible divergence inside that scope is REQUIRED here whichever stage wrote the line that diverges. The criterion is read as the claim, not as the artifact: *"conformance covers `emitLint` and `inspect` agreeing"* is not met by a green suite asserting they agree while they do not. A stage that may hand back every defect its own integration test existed to surface is a stage that only ever has to write tests that pass.

**What bounds that is the claimed scope, written before you probe** — `rebuild-the-picture.md`'s question 6. Without it this rule deletes FOLLOW-UP instead of narrowing it, because a scope drawn after a finding always contains the finding, and one drawn after deciding not to raise it never does.

**PASS asserts something narrow and exact**: *the checks I ran, listed, passed; the probe classes I ruled out, listed with the reason, are genuinely not triggered by this change; the claimed scope is stated and nothing inside it is left unproven; the axes in play are listed with their values and every pair they make is crossed or ruled out one by one; and every claim in every implementer's report is placed in a bucket.* **The dispatcher checks those blocks are present before it reads the verdict word, and returns a PASS missing one as `VOID`** — deliver-ticket's `deliver-a-stage.md`, *Reading the verdict*. It does not assert the stage is good, that the ticket is on track, or that a later stage will work. **A check you did not run is not a pass** — an acceptance criterion with no command, output or artifact that can show it is a REQUIRED finding, not a judgement call. deliver-ticket's `finish-the-ticket.md` holds the ticket's own goal to that same bar one level up.
...
## Doubt the instrument before the artifact

**When a probe says the work is wrong, the likeliest explanation is that the probe is wrong.** You wrote it a moment ago, against text you were looking at; the work has been through an implementer and a dispatcher. **A zero on one side against a one on the other is a question, not a result.**

**So run every probe at both ends before it becomes a finding** — where the thing should be absent and where it should be present. A probe that cannot tell those apart has told you nothing, whichever way it came out, and it is one extra run of a command you have already written.

**#387 recorded twenty-two instrument failures, every one a probe rather than an artifact, and none caught by the check that produced it.** Several would have destroyed correct work: a repaired file reported as still broken, because the repair appended and the old string survived inside the new one; a false `VOID` on a sound review, from a regex built in a double-quoted string where it executed instead of matching; a false zero on a quotation wrapped across two lines. **An empty result and a clean result look identical, and so do a hit and a stale substring** — so the discriminating anchor is the part that *changed*, never the part that stayed.

**This is not a licence to disbelieve a finding you dislike.** The rule is about the order of the two checks, not about which answer wins: measure the instrument first, then report whatever it says.

**`deliver-ticket/SKILL.md` states this same rule under the same heading, and the two move together or not at all.** Two copies, because you never load that page and the dispatcher never loads this one, and the rule has to be in hand at the moment a probe returns rather than one skill away — the same reason the comment-authorship convention is held in two places. **A change to either is a change to both.**
