...
**PASS asserts something narrow and exact**: *the checks I ran, listed, passed; the probe classes I ruled out, listed with the reason, are genuinely not triggered by this change; the claimed scope is stated and nothing inside it is left unproven; every dimension pair this change puts in contact is crossed or ruled out one by one; and every claim in the implementer's report is placed in a bucket.* It does not assert the stage is good, that the ticket is on track, or that a later stage will work. **A check you did not run is not a pass** — an acceptance criterion with no command, output or artifact that can show it is a REQUIRED finding, not a judgement call. deliver-ticket's `finish-the-ticket.md` holds the ticket's own goal to that same bar one level up.
