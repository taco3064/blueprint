...
function walk(dir) {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    else if (entry.name.endsWith('.test.ts')) migrateFile(full);
  }
}
...
function migrateFile(file) {
  ...
  function architectureMigration(node) {
    ...
    const directLayout = property(node, 'layout');

    if (!layersProp || !moduleProp || !ts.isPropertyAssignment(moduleProp)
      || !ts.isObjectLiteralExpression(moduleProp.initializer)) {
      if (directLayout && ts.isPropertyAssignment(directLayout)
        && stringValue(directLayout.initializer) === 'flat') {
        return replaceProperty(node, directLayout,
    ...
    const properties = node.properties
      .filter((candidate) => candidate !== moduleProp)
      .map((candidate) => {
  ...
  function layerMigration(node, shared) {
    ...
    if (looksLayer && shape) {
      ...
      if (layout === 'folder') {
        properties.push(
          ts.factory.createPropertyAssignment('layout', ts.factory.createStringLiteral('folder')),
        );

        if (shape.entry && shape.entry !== 'index') {
          properties.push(
            ts.factory.createPropertyAssignment('entry', ts.factory.createStringLiteral(shape.entry)),
          );
        }
      }
    ...
    return {
      changed,
      node: changed ? ts.factory.updateObjectLiteralExpression(node, properties) : node,
    };
  }
