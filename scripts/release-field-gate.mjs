...
export function validateReleaseEvidence({ repository, sha, combinedStatus, comment, ticket }) {
  ...
  // A green status is only a pointer; the evidence is the comment it names, on a ticket still open
  if (comment.html_url !== status.target_url) throw new Error('Field convergence comment is not the one the status links to.');
  if (ticket?.state !== 'open') throw new Error(`Convergence ticket #${target[1]} is not open.`);
...
function main() {
  ...
  const status = (combinedStatus.statuses ?? []).find((entry) => entry.context === FIELD_CONTEXT);
  const target = /\/issues\/(\d+)#issuecomment-(\d+)$/.exec(status?.target_url ?? '');

  if (!target) validateReleaseEvidence({ repository, sha, combinedStatus, comment: { body: '' } });

  const comment = gh([`repos/${repository}/issues/comments/${target[2]}`]);
  const ticket = gh([`repos/${repository}/issues/${target[1]}`]);
  const result = validateReleaseEvidence({ repository, sha, combinedStatus, comment, ticket });
