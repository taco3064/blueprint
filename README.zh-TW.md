...
「動這個會炸到誰？」—— 改 legacy code 前先問這句。target 給 module key 或檔案路徑都行。
...
程式化的 `blueprint inspect`。`options`：`{ framework?, json?, log?, loadConfig?, baseline?, updateBaseline? }`。每個 `Finding` 是 `{ severity: 'error' | 'warn' | 'info', rule, path, message }`；有任何 error 級 finding 時 `ok` 為 `false`。baseline 模式下 `findings` 只裝新增（非 baseline）的那些。

### `runDeps(root, options?): Promise<{ ok, modules }>`

程式化的 `blueprint deps`。`options`：`{ target?, framework?, json?, log?, loadConfig? }`。每個 `ModuleDeps` 是 `{ module, importedBy, imports }`，按 fan-in 降冪排序；`target` 找不到時 `ok` 為 `false`。
...
| `blueprint inspect` | `--framework vue\|react` · `--json` · `--baseline` · `--update-baseline` | 有 error 級 finding 時 `1`（`--baseline` 下只看新增的） |
| `blueprint deps [module]` | `--framework vue\|react` · `--json` | module 找不到時 `1` |
